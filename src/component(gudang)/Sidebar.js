 ...
 * - onOpenTambah?: () => void
 */
export default function Sidebar({ view, setView, onOpenTambah }) {
  const [showLogout, setShowLogout] = useState(false);
  const [openMobile, setOpenMobile] = useState(false);
  const navigate = useNavigate();

  const navBtn = "w-full text-left px-3 py-2 rounded-xl font-medium transition-colors";
  const activeBtn = "bg-green-100 text-green-800";
  const normalBtn = "hover:bg-neutral-100 text-neutral-700";
  ...
  const Nav = ({ onClick }) => (
    <nav className="space-y-1">
      <button
        onClick={() => {
          setView?.("dashboard"); // satu-satunya halaman
          onClick?.();
        }}
        className={`${navBtn} ${view === "dashboard" ? activeBtn : normalBtn}`}
      >
