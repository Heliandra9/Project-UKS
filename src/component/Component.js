...
function SideBar(props) {
  ...
  return (
    <div
      className={`w-full md:w-auto h-fit sm:h-screen bg-green-500 text-white ${
        props.class || ""
      }`}
    >
      <div className="p-2 md:p-4">
        <div className="flex flex-col sm:flex-row mb-2 md:mb-4 items-center">
          <img
            ...
            alt="Logo"
            className={`${
              props.nav && props.hober ? "w-8 h-8" : "w-12 h-12 md:w-18 md:h-18"
            } sm:my-0 mb-2 mr-2`}
          />
          <div
        ...
        <ul className="flex sm:flex-col justify-center sm:justify-start md:space-y-2 h-12 md:h-screen relative">
          ...
          {items.map((item, index) => (
            <li
              key={index}
              className={`flex z-10 items-center p-2 rounded-lg cursor-pointer transition-all duration-300 ease-in-out border-0 border-green-500 ${
                item.class || ""
              }`}
            ...
            >
              <i
                className={`${item.icon} mr-2 ${
                  props.nav && props.hober ? "absolute right-0" : ""
                }`}
              ></i>
              <span
                className={`
...
function Button(props) {
  return (
    <button
      ...
      className={`bg-${props.color} cursor-pointer hover:opacity-50 ${
        props.sizeTxT ? props.sizeTxT : ""
      } ${props.width ? props.width : "w-full"} p-2 text-${
        props.textColor
  ...
  return (
    <div
      className={`fixed inset-0 flex items-center justify-center z-50 ${
        props.stat ? "block" : "hidden"
      } backdrop-blur-sm`}
    >
      <div className="bg-white relative rounded-xl shadow-2xl p-8 w-full max-w-lg">
              ...
              <div key={field.name} className="space-y-2">
                ...
                {field.name === "golongan_darah" ? (
                  ...
                  >
                    ...
                    {["Tablet", "Sirup", "Kapsul", "Salep"].map((item) => (
                      <option key={item} value={item}>
                        {item}
                      </option>
                    ))}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(form.obat || []).map((obat, idx) => (
                        ...
                        >
                          ...
                          />
                          <span className="ml-2 text-sm">{obat.jenis_obat}</span>{" "}
                          {/* Menambahkan jenis_obat */}
                          <button
                            type="button"
                            onClick={() => {
                              const newObat = form.obat.filter(
                                (_, i) => i !== idx
                              );
                              setForm({ ...form, obat: newObat });
                            }}
                          ...
                          return (
                                  ...
                                  setForm({
                                    ...
                                    obat: [
                                      ...(form.obat || []),
                                      {
                                        ...
                                        jumlah: 1,
                                        jenis_obat:
                                          obat.jenis_obat === "Sirup"
                                            ? "ml"
                                            : "pcs",
                                      },
                                    ], // Menambahkan jenis_obat otomatis
                                  });
                                }
                              }}
                              className={`flex justify-between items-center px-3 py-2 rounded-lg cursor-pointer transition-all mb-1 ${
                                isSelected
                            ...
                            >
                              ...
                              <span className="text-xs text-gray-500">
                                Stok: {obat.stock_obat}{obat.satuan}
                              </span>
                            </div>
                          );
                        })
        ...
        <button
          ...
          className={`w-full py-3 rounded-md text-white ${
            props.name === "edit" || props.name === "insert"
              ? "bg-blue-600"
              : "bg-red-600"
          } hover:bg-opacity-90`}
        >
